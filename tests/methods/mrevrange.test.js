...
describe('mrange method tests', () => {
  ...
  it('should fetch range of multiple time series', async () => {
    ...
    const query = [TS_MREVRANGE, fromTimestamp, toTimestamp, FILTER, `${label1}!=`, `${label2}=`];

    await rts
      .mrevrange(fromTimestamp, toTimestamp)
      .filter([
  ...
  it('should fetch N items in range of multiple time series', async () => {
    ...
    const query = [TS_MREVRANGE, fromTimestamp, toTimestamp, COUNT, count, filterQuery];
  ...
  it('should fetch N items in range of time series for given key and aggregation', async () => {
    ...
    const query = [TS_MREVRANGE, fromTimestamp, toTimestamp, COUNT, count, ...aggregationQuery, filterQuery];
  ...
  it('should fetch range of time series with labels for given key and aggregation', async () => {
    ...
    const filterQuery = `${FILTER} ${label1}!=${value1}`;
    const aggregationQuery = [AGGREGATION, type, timeBucket];
    const query = [TS_MREVRANGE, fromTimestamp, toTimestamp, ...aggregationQuery, WITHLABELS, filterQuery];
  ...
  it('should fail, no arguments', async () => {
    await expect(rts.mrevrange().send()).rejects.toThrow();
  });

  it('should fail, no toTimestamp', async () => {
    const { fromTimestamp } = TEST_PARAMS;
    await expect(rts.mrevrange(fromTimestamp).send()).rejects.toThrow();
  });

  it('should fail, no filter', async () => {
    const { fromTimestamp, toTimestamp } = TEST_PARAMS;
    await expect(rts.mrevrange(fromTimestamp, toTimestamp).send()).rejects.toThrow();
  });

  it('should fail, filter is empty', async () => {
    const { fromTimestamp, toTimestamp } = TEST_PARAMS;
    await expect(rts.mrevrange(fromTimestamp, toTimestamp).filter().send()).rejects.toThrow();
  });
